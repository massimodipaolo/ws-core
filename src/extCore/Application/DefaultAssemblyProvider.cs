...
public class DefaultAssemblyProvider : IAssemblyProvider
{
    ...
    {
        var _ctx = DependencyContext.Default.CompileLibraries?.Where(IsCandidateCompilationLibrary)?.Select(_ => _.Name);
        if (_ctx != null)
            foreach (string assemblyName in _ctx)
            {
    ...
    {
        if (includingSubpaths && !string.IsNullOrEmpty(path))
